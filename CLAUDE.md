...
- **The `ui/initialize` result is not empty.** It carries `hostContext` with the active `theme` and a `styles.variables` palette, refreshed later by a `ui/notifications/host-context-changed` notification — which has a method and no id, so it matches neither branch of a listener written only for replies and host-initiated requests. Both are handled now. Every colour is `var(--color-…, <Delta fallback>)`, so a host that sends nothing still gets Delta's neutrals.
