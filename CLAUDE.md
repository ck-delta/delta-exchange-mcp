...
7. **Hot rebind is one atomic state swap** — `_request` captures `_ClientState` before its first await, and `rebind()` replaces the config, signing prefix, and transport together. Old transports stay open until server shutdown so an in-flight request cannot be closed underneath itself.
