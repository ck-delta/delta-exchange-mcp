...
- **`preferredSize` is not a field.** It does not appear anywhere in the spec types; the height is whatever the view reports. `prefersBorder` *is* a field, and omitting it is what produced two nested boxes — the host drew one and the view drew another inside it. The resource asks for `prefersBorder: True` and the view draws none, which also degrades correctly on a host that draws nothing, because the input fills carry the structure.
...
**Layout and dimension are the host's, not the view's.** The stylesheet names no pixel type size, no width and no spacing scale. Type comes from the `--font-text-*` / `--font-heading-*` / `--font-weight-*` tokens; one `--gap: 1em` is the only spacing step, so it tracks whatever type the host asked for; and the fields, radios, checkbox and select are left as the platform draws them, with `color-scheme` doing the work of making a native control render correctly inside a dark client. An earlier version restyled every field with its own padding, border, background and a `14px` base, and that is what looked wrong in Codex — the numbers were right for an assumed base and Codex does not run one. Measured with the host's own tokens applied: the same content reports 437px at the 16px default and 381px at a host-supplied 14px, with no rule re-tuned. Regression test: `test_the_view_names_no_type_size_of_its_own` fails on a px literal in any font declaration. Only three things override anything now: the brand-filled button (a native one loses the primary action), `accent-color` on the radios and checkbox, and the two flex rows that are genuinely structural — logo beside title, checkbox opposite link.
